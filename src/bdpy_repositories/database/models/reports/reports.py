...
from sqlalchemy import Column, Integer, BigInteger, Timestamp, Tinyint, UniqueConstraint, ForeignKey
...
class Reports(Base):
    ...
    __table_args__ = (
        UniqueConstraint('reportedID', 'reportingID', 'region_id', 'manual_detect', name='Unique_Report'),
    )
    ...
    # Foreign Keys
    reportingID_fk = ForeignKey('Players.id', name='FK_Reporting_Players_id', ondelete='RESTRICT', onupdate='RESTRICT')
    reportedID_fk = ForeignKey('Players.id', name='FK_Reported_Players_id', ondelete='RESTRICT', onupdate='RESTRICT')
